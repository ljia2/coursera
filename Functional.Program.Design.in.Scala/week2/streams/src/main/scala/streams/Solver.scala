...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    b.legalNeighbors.map{case (n, m) => (n, m::history)}.toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty){
      ...
      val newNeighbors = initial.flatMap{case (block, history) => newNeighborsOnly(neighborsWithHistory(block, history) , explored)}
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.map(_._2).headOption.getOrElse(List()).reverse
}
