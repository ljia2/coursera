...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    ...
    if(!left.isEmpty && !right.isEmpty)
      if(leftMostRetweeted.retweets > rightMostRetweeted.retweets)
        if(leftMostRetweeted.retweets > elem.retweets) leftMostRetweeted else elem
      else
        if(rightMostRetweeted.retweets > elem.retweets) rightMostRetweeted else elem
