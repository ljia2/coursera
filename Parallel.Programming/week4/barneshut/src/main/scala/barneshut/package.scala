...
package object barneshut {
  ...
  extends Quad {
    ...
    val massX = if(mass > 0) bodies.map(b => b.mass * b.x).sum / mass else centerX
    val massY = if(mass > 0) bodies.map(b => b.mass * b.y).sum / mass else centerY
    val total: Int = bodies.length
    def insert(b: Body): Quad = {
      if(size <= minimumSize)
        new Leaf(centerX, centerY, size, b +: bodies)
      else {
        val newFork = new Fork(new Empty(centerX - size/4, centerY - size/4, size/2),
          new Empty(centerX + size/4, centerY - size/4, size/2),
          new Empty(centerX - size/4, centerY + size/4, size/2),
          new Empty(centerX + size/4, centerY + size/4, size/2))
        (bodies :+ b).map(newFork.insert(_))
        newFork
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for(i <- 0 until this.matrix.length) yield matrix.update(i, matrix(i).combine(that.matrix(i)))
      this
    }
