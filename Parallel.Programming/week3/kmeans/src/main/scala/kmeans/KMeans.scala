...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val classifyResults = points.map(p => (findClosest(p, means), p)).groupBy(_._1).mapValues(_.map(_._2))
    means.map{mean => mean -> classifyResults.getOrElse(mean, GenSeq.empty[Point])}.toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(oldMean => findAverage(oldMean, classified.getOrElse(oldMean, GenSeq())))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (oldMeans zip newMeans).filterNot{case (oldMean, newMean) => oldMean.squareDistance(newMean) <= eta}.isEmpty
  }
