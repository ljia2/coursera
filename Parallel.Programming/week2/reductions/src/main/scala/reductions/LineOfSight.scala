...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while(index < until){
      if(max < input(index)/index) max = input(index)/index
      index = index + 1
    }
    max
  }
   ...
   *  array is smaller or equal to `threshold`, and a `Node` otherwi` otherwisese.
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    while(index < until){
      val angle = input(index)/index
      if(angle > maxAngle) maxAngle = angle
      output(index) = maxAngle
      index = index + 1
    }
  }
